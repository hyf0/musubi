...
- **Existing `Language` boundary:** Keep the BCP 47 `Language` setting for document language metadata and locale-sensitive formatting such as publication dates. In the initial version it does not select a translated Musubi interface, and accepting a valid non-English tag must not be described as built-in interface-language support. When the document `Language` is not English, mark Musubi-owned English interface strings or an appropriate shared container with `lang="en"` so assistive technology does not pronounce them as the document language; do not override the language of owner-authored content.
...
- **Date:** Initial delivery selected 2026-07-22; runtime shard strategy approved 2026-07-23.
- **Status:** Required.
- **Scope:** Generated Tsanger JinKai webfont artifacts for Musubi's static output and the complete browser-time Chinese fallback supplied by LXGW WenKai GB. The original Tsanger and LXGW TTF files remain private build inputs.
- **Shared subset corpus:** Generate content-derived WOFF2 subsets instead of shipping the complete W04 and W05 sources. Build one complete Chinese typography corpus from every published Post, Page, optional Home, site setting, navigation item, metadata field, rendered date, finite Musubi-owned interface string, and Chinese code character. Give that same requested code-point set to both W04 and W05; each output keeps every requested mapping its source supports. Visual components still choose W04 or W05 according to the accepted typography roles, but subset correctness must not depend on independently reconstructing those roles. Do not add Latin text or code symbols assigned to Charter or JetBrains Mono.
- **Contextual punctuation:** Han characters, CJK punctuation blocks, and full-width forms are unconditionally Chinese typography. Middle dots, em dashes, curly quotes, ellipses, and the other explicitly shared punctuation follow the closest Han or non-Han text within the same text run, preferring the preceding script only when both sides are equally close. Rendering and corpus generation must use the same classifier. Inline emphasis, deletion, and link markup remain within one run; a visual line break or inline-code boundary ends it. This keeps English punctuation from triggering Tsanger without losing Chinese punctuation across presentational markup.
- **Fallback delivery:** Pin `LXGW WenKai GB Medium v1.522` as the source for `Musubi CJK Fallback`. Generate 32 checked-in WOFF2 shards containing every mapping from that source that Musubi's Chinese typography classifier can ever select: ten common-character shards derived from a fixed, checksummed snapshot of Noto Serif SC v35's measured production delivery groups, followed by 22 shards that split the remaining mappings inside the existing Unicode regions at no more than 1536 code points each. The snapshot stores final code-point sets and provenance only; ordinary setup, build, and publication never request Google Fonts. Exclude ASCII, ordinary Latin, and every other mapping that cannot enter Chinese typography, keep every generated shard at or below 600,000 bytes, and publish all 32 shards with every site build after Tsanger in the family stacks. `unicode-range` lets the browser fetch a shard only when the preferred Tsanger subset does not cover a rendered character. Use the same Medium outlines for weights 400 through 500 rather than synthesizing another weight or introducing a second fallback typeface.
- **Runtime boundary:** The complete CJK-only LXGW pool is an intentional browser-time fallback, including for Chinese text introduced after the font subsets were built or omitted from the static corpus. Such text must use LXGW rather than an unrelated system face whenever the pinned LXGW source contains it. Characters outside Musubi's Chinese typography universe continue through the ordinary Latin, symbol, or generic system stacks.
- **Regeneration, caching, and validation:** Every static build derives or validates its content subsets against the complete Site, source fonts, pinned tools, fixed hot-shard data, and complete runtime fallback. Generated WOFF2 files and the generated font stylesheet use content-addressed URLs with immutable caching. Both Tsanger subsets must be requested from the same Chinese corpus; a requested mapping supported by a source must survive its subset. Every build-known Chinese mapping must exist in the published LXGW runtime pool even when Tsanger also supplies it, and absence from that pool stops the build. The full quality gate decodes every checked-in shard, verifies its exact expected group, file hash, byte ceiling, non-overlapping coverage, and absence of mappings that the Chinese classifier can never select.
- **Fallback licensing and identity:** Keep generated LXGW subsets separate from Tsanger, use the non-reserved internal family name `Musubi CJK Fallback`, retain copyright and SIL Open Font License 1.1 metadata, ship the OFL text, and pin the official [`LXGWWenKaiGB-Medium.ttf` release `v1.522`](https://github.com/lxgw/LxgwWenKaiGB/releases/download/v1.522/LXGWWenKaiGB-Medium.ttf) at SHA-256 `b885c51ec0d3f325974013801dfcefda1a9ba0bf385c607cf5f2582dafa2e5ab`.
- **Reason or reference:** Yunfei explicitly chose the shared W04/W05 corpus on 2026-07-22 because the small duplicated subset cost is preferable to making delivery correctness depend on every visual role and corpus role staying synchronized. He then explicitly corrected the fallback boundary on the same date: Musubi must publish a complete Chinese fallback even though it adds about 9 MB to the static artifact, because normal pages do not fetch shards whose characters Tsanger already covers and Chinese introduced after the build must not silently switch to a system face. The review that prompted this work found both a role-split table-header miss and ASCII exposed by the old full LXGW files; the shared Tsanger corpus fixes the former, while filtering the complete runtime pool to classifier-reachable CJK mappings fixes the latter without giving up runtime coverage. On 2026-07-23 Yunfei explicitly approved and vouched the measured 32-shard recommendation after comparison with the eight-shard baseline, mechanical 25-shard splitting, and a finer 60-shard alternative: the selected strategy substantially reduces ordinary fallback downloads and caps rare-character cost while avoiding the additional files and smaller marginal network gain of the finer alternative. LXGW Medium remains selected because its Simplified Chinese forms and density are compatible with the accepted Tsanger pairing.
...
### Latin and code font delivery

- **Date:** 2026-07-22.
- **Status:** Required.
- **Scope:** Delivery of the already-selected Charter English family and JetBrains Mono code family; this does not change their visual roles or introduce another family.
- **Decision:** Self-host content-derived WOFF2 subsets so the selected faces do not depend on fonts installed on the reader's operating system. Pin and verify Charter regular, italic, bold, and bold italic from the unrestricted Bitstream Charter webfont package, and JetBrains Mono regular, italic, bold, and bold italic from official release `v2.304`. Keep source WOFF2 files in the private setup cache, publish only generated content-addressed subsets, ship each upstream license, and declare the exact style and weight for every face. Preserve the four JetBrains combinations because Shiki tokens can request bold and italic together while `font-synthesis: none` forbids a synthetic substitute.
- **Loading:** Use `font-display: swap` and immutable caching for the content-addressed font files and stylesheet. Do not blanket-preload every family or style; a browser should request only faces actually used on the current page.
- **Reason or reference:** Yunfei accepted the delivery review on 2026-07-22. The prior CSS named Charter and JetBrains Mono without supplying either family, so appearance varied by operating system and the LXGW fallback could intercept Latin or code before the generic fallback.
...
- **Date:** 2026-07-18.
- **Status:** Required for the initial implementation.
